...
- **Table** — a documentation table with grouped rows, built-in sorting and search, cell text formatting (bold, italic, links), and four layout presets: Standard, Field List (a compact two-column layout on mobile), Catalog Card (renders rows as an image/title/detail card grid), and Plain (a minimal, WordPress-core-style table with per-column text alignment). Inserting a new Table starts with a column/row count prompt, like the core Table block.
